...
while True:
    ...
    if choice == "load":
        ...
        try:
            recordf = open("record.txt","r")
            count = recordf.read()
            recordf.close()            
            print(f"Police win {count.count('P')} round")
            print(f"Robber win {count.count('W')} round\n")
        except:
            print("Police win 0 round\nRobber win 0 round")
    elif choice == "clear":
...
while lose is False and win is False:
    ...
    if map["turn"] == "P":
        ...
        polinput = str(input("Please select a policeman(A,B,C) ")) #select a police
        while polinput not in police: #select error
            if polinput not in police:
                polinput = str(input('Invalid policeman! Please select policeman A, B, or C or enter \"save\" to save the current file '))
